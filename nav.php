<div id="main">
<div id="content">
<nav class="menu">
    <ul>
        <li><a href="?link=avaleht.php">Avaleht</a></li>
        <li><a href="?link=RaamatJS.php">Raamatukogu JS</a></li>
        <li><a href="?link=JSmuusika.php">Muusika Küs JS</a>
        <li><a href="#">Funktsioonid</a>
            <ul class="dropdown">
                <li><a href="?link=ajafunk.php">Ajafunktsioonid</a></li>
                <li><a href="?link=textfunkts.php">Textfuntsioonid</a></li>
                <li><a href="?link=matem_funkts.php">Matemaatika funktsioonid</a></li>
                <li><a href="?link=pildid.php">Töö pildifailidega</a></li>


            </ul>
        </li>
        <li><a href="#">Mobilli mall</a>
            <ul class="dropdown">
                <li><a href="content/mobilimail/p2is.php">Mobillimalli järgi veebilehestik(näide)</a></li>
                <li><a href="content/mobilimail/anekdoot/anekdotleh.php" target="_blank">Anekdootid</a></li>
            </ul>
        </li>
        <li><a href="?link=gitKasutamine.php">Git kasud</a>
        </li>
        <li><a href="https://anastasiiaradasheva24.thkit.ee/" target="_blank">Vana index</a></li>
    </ul>
</nav>
</div>
</div>